...
export const NameCell = ({ rowData, dataKey, ...props }: CellProps) => {
  const speaker = (
    <Popover title="More Infor">
      <p>
        <b>Done:</b> {rowData.timeDone}
      </p>
    </Popover>
  );
  ...
  return (
    <Cell {...props}>
      <Whisper placement="top" speaker={speaker}>
        <a href="/">{dataKey ? rowData[dataKey] : null}</a>
      </Whisper>
    </Cell>
  );
};
...
export const ImageCell = ({ rowData, dataKey, ...props }: CellProps) => (
  <Cell {...props} style={{ padding: 0 }}>
    ...
    >
      <img src={rowData[dataKey!]} width="40" alt='/' />
    </div>
  </Cell>
);
  ...
  return (
    <Popover ref={ref} className={className} style={{ left, top }} full>
      <Dropdown.Menu onSelect={handleSelect}>
        <Dropdown.Item eventKey={1}>View more</Dropdown.Item>
      </Dropdown.Menu>
    </Popover>
  );
};
