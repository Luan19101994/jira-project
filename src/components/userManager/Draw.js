...
function Draw() {
  ...
  const category = ["Dự án web", "Dự án app"].map((item) => ({
    label: item,
    value: item,
  }));
  const submit = async () => {
    ...
    <Drawer open={showDetail} onClose={() => setShowDetail(false)}>
      <Drawer.Body>
        ...
        <div className="mt-4 flex items-center justify-end gap-3">
          <button
            className="py-2 px-4 text-black flex items-center justify-center border-[lightgrey] bg-[#fff] rounded-md"
            onClick={() => submit()}
          >
            Cancel
          </button>
          <button
            className="py-2 px-4 text-white flex items-center justify-center border-[#1677ff] bg-[#1677ff] rounded-md"
            onClick={() => submit()}
          >
            Submit
          </button>
